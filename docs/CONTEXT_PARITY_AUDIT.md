...
| **Scoped** | The source surface is understood but deliberately excluded from the ported readers. It is not silently counted as migrated. |
| **Pending** | The source data surface has not yet been ported. This says nothing by itself about pointer availability. |
| **Pointer unavailable** | The source data surface and supplied-address reader are ported, but Stealth does not yet have its own route to obtain the runtime pointer. Callback-owned pointers are tracked in [`CALLBACK_POINTER_RESEARCH.md`](CALLBACK_POINTER_RESEARCH.md). The two map contexts that were in this state now have a route: the UI frame array. |
...
must exist in the Stealth API. When an operation's mechanism is not ported, it
remains declared and is marked with what it needs; it is not
...
- **Runtime limits, not missing context ports:** automatic `MapContext`
  callback registration is not ported, and the two callback-owned map contexts
  now take their root from the client's UI frame array instead of a callback.
  The Stealth-owned hook/callback layer exists in
  `py4gw/game_thread/`; nothing consumes it for these contexts, and the project
  still does not rely on Reforged's DLL/shared-memory publication. See
...
| --- | --- | --- | --- | --- | --- |
| `CharContext` | `character.h`, `GetCharContext()` | `CharContext.py` | **Declaration parity PASS; runtime scoped** | Full maintained `CharContextStruct`, observer matches, progress bar, fixed strings, arrays, the GameContext pointer path, and the source facade methods. | `enable` registers an in-process callback the source owns; no ported context is wired to Stealth's own callback layer yet, so it remains declared and reports an explicit unsupported operation. |
...
| `PreGameContext` | `pregame.h`, `GetPreGameContext()` | `PreGameContext.py`, `PreGameContext.pyi` | **Declaration parity PASS; runtime externally verified** | Full layout and login-character array, character-name helpers, source facade members, and remote array binding. | The source callback registration/shared-memory publication is not yet ported; it needs the in-process registration path (next). The declarations remain present and report that explicitly. |
| `Cinematic` | `cinematic.h`, `game.h` `cinematic` field | `CinematicContext.py`, `CinematicContext.pyi` | **Declaration parity PASS; runtime externally verified** | Full 8-byte structure, source facade members, and `GameContext.cinematic` pointer relationship. | The source callback/shared-memory publication is not yet ported; it needs the in-process publication path (next). Declarations remain present and report that explicitly. |
| `GameplayContext` | `gameplay.h`, `context_methods.cpp` global pointer | `GameplayContext.py`, `GameplayContext.pyi` | **Declaration parity PASS; runtime externally verified** | Full `0x78`-byte structure, source facade members, resolver, and mission-map zoom field. | The source callback/shared-memory publication is not yet ported; it needs the in-process publication path (next). Declarations remain present and report that explicitly. |
| `ServerRegion` | `context.cpp`, `context_methods.cpp`, `GetRegionIdPtr()` | `ServerRegionContext.py`, `ServerRegionContext.pyi` | **Declaration parity PASS; runtime externally verified** | Four-byte signed region value, source facade members, and cached JSON resolver. | The source callback/shared-memory publication is not yet ported; it needs the in-process publication path (next). Declarations remain present and report that explicitly. |
| `InstanceInfo` | `map.h`, `context.cpp`, `context_methods.cpp`, `GetInstanceInfoPtr()` | `InstanceInfoContext.py`, `InstanceInfoContext.pyi` | **Declaration parity PASS; runtime externally verified** | `InstanceInfo`, `MapDimensions`, and `AreaInfo` layouts, nested remote reads, all source flag/file-ID properties, aliases, and facade members. | The source callback/shared-memory publication and legacy in-process symbol call are not yet ported; they need the in-process publication and symbol-call path (next). Declarations remain present and report that explicitly. |
| `TextParser` | `text_parser.h`, `game.h` `text_parser` field | `TextContext.py`, `TextContext.pyi` | **Declaration parity PASS; runtime externally verified** | Exact source root field order and sizes, typed language/file-slot records, source properties/helpers, source facade members, bounded remote pointer reads, and the source's own string-table trigger on the first refresh (`_update_ptr` → `_do_load_string_table`, `TextContext.py:152-155`). Live: 11 language slots × 99 file slots, `entries_per_file` 1024. | Callback publication (the in-process `enable()` registration) is not yet ported; declarations remain present and report that explicitly. |
| `AvailableCharacterArray` | `account.h`, `player_patterns.cpp`, `GetAvailableCharactersPtr()` | `AvailableCharacterContext.py`, `AvailableCharacterContext.pyi` | **Declaration parity PASS; runtime externally verified** | Exact `0x84` entry and `0x10` array-header layouts, all packed properties, source names/aliases, bounded value traversal, and facade members. | The source callback/shared-memory publication is not yet ported; it needs the in-process publication path (next). Declarations remain present and report that explicitly. |
| `PartyContext` | `party.h`, `GetPartyContext()` | `PartyContext.py`, `PartyContext.pyi` | **Declaration parity PASS; runtime externally verified** | Exact root and nested record layouts, native party-search enum, player/hero/henchman members, native helper aliases, party arrays, invite-list traversal, party searches, flag/text helpers, source record aliases, and source facade members including `_cached_ptr`. | Callback registration is not yet ported; it needs the in-process registration path (next), even though Stealth now has its own callback layer in `py4gw/game_thread/`. No selected source declaration is omitted. |
...
| `AccAgentContext` | `agent.h`, `GetAgentContext()` | `AccAgentContext.py`, `AccAgentContext.pyi` | **Declaration parity PASS; runtime externally verified** | Exact root, summary, extension, movement, and native-only `AgentInfo` layouts; all source array properties and aliases; source facade declarations; nested gadget-name reads. | The injected callback is not yet ported here. Native `AgentInfoArray` is declared in `agent.h` but has no field in `AgentContext` or getter in `context_methods.cpp`; its pointer source remains unresolved and no connection is inferred. |
| `AgentArray` | `agent.h`, `GetAgentArray()` | `AgentContext.py`, `AgentContext.pyi`, `AgentArray.py`; `Agent.py` helpers are a separate surface | **Structure and traversal slice verified; Reforged helper parity partial; callback lifecycle not ported** | Common/living/item/gadget layouts, equipment/item/tag/effect records, source value dataclasses and snapshot conversions, `AgentArrayStruct` fields/cache methods, bounded pointer-table traversal, movement validity checks, category methods, stale-reference validation, typed records, corpse-state/signature properties, source item-facade method names, and source pure-list helpers (`Manipulation`, `Sort.ByCondition`, `Filter.ByCondition`). Python/native layout disagreements have separate explicit declarations. | The source cache gate checks corresponding external MapContext, CharContext, InstanceInfo, WorldContext, and AccAgentContext readers plus the source conditions. Cache records are materialized through remote reads instead of in-process pointers. The 300-reference `GetAgentByID` lookup is served from the same bounded, validated current array; this replaces the Reforged shared-memory transport rather than omitting its lookup behavior. Callback registration is not yet ported; it needs the in-process registration path (next). Process-wide Reforged state is adapted to per-client ownership. Agent-dependent `Sort.ByAttribute`, `Sort.ByDistance`, `Sort.ByHealth`, `Filter.ByAttribute`, `Filter.ByDistance`, and `Routines.DetectLargestAgentCluster` are not ported because they rely on the separate `Agent.py` query surface, which is unaudited and not present in Stealth. The native root is already represented by the `AccAgentContext` row. |
| `Camera` | `camera.h`, `GetCamera()` | Native camera layout; Reforged `Camera.py` facade; native camera bindings | **Declaration parity PASS; read getters verified; actions declared but not ported** | Exact native fields and read methods, all `Camera.py` facade getter names, `IsPointInFOV`, fixed-width pointer treatment, JSON resolver, and bounded read. | Source action names are declared but not yet ported; they need the game-thread call form (next), because their bindings enqueue work in the game thread or mutate camera state. This member performs no camera writes and makes no game-thread call. |
...
| `FriendList` | `friend_list.h`, `GetFriendList()` | `PyFriendList.pyi` and native friend-list bindings | **Declaration parity PASS; live read verified; game-thread actions declared but not ported** | Exact `Friend`, `FriendList`, and flexible `FriendEventData` header fields; enum values; bounded friend traversal; source count/status APIs and action names. | `set_friend_list_status`, `add_friend`, and `add_ignore` are declared but not yet ported; they need the game-thread action path (next), because the native bindings enqueue game-thread actions. Native `RemoveFriend` is not exposed by `PyFriendList.pyi` and is outside the Python facade audited here. |
...
| `SalvageSessionInfo` (`item.h`, `context/item.h`, `src/GW/item/item.cpp`) | The complete 0x24 native record and all nine field names (`vtable`, `frame_id`, `item_id`, `salvagable_1/2/3`, `chosen_salvagable`, `h001c`, `kit_id`) are declared and offline-tested, with a bounded `read_at(reader, address)`. The root is acquired read-only by walking the client's UI frame array to the frame that registered the salvage-popup callback, cross-checked against the record's own `frame_id` at `+0x4`. The facade is Stealth-owned, because Reforged Python has no reader for this structure. | The open/close handoff is untested: the frame does not exist while the window is closed, so a test needs an operator to open the salvage window. `enable`/`_update_ptr` are not yet ported; they need the in-process callback registration path (next). Neither reference project consumes this record, so it is not a pointer-acquisition gap. See [`UI_FRAME_TREE.md`](UI_FRAME_TREE.md). |
...
callbacks/shared memory, which this project does not reproduce; instead it reads the
same pointers out of the client's UI frame array, and both contexts are live-verified
open and closed that way. That is a boundary difference in how the pointer is
obtained, not unfinished context porting. `GwDxContext` is native render state and is
outside the required in-game context migration.
...
`declared but not ported`: each is not yet ported and names the mechanism it needs, and the
target-side layer those mechanisms would use now exists in `py4gw/game_thread/`
without any of them being wired to it. They are
not silently omitted from the source port. The complete outstanding work list and its
...
[`PATHING_MIGRATION_PLAN.md`](PATHING_MIGRATION_PLAN.md). The two callback-owned map
contexts are now resolved read-only through the UI frame array; render pointer
acquisition still needs target-side code. UI support APIs,
...
helpers remain unported: they are declared with the mechanism they need, rather than being part of
the read parity this audit measures.
