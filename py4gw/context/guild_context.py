...
class GHKeyStruct(Structure):
    """The native 0x10 guild-hall key."""

    _pack_ = 1
    _fields_ = [("key_data", c_uint32 * 4)]

    @property
    def as_string(self) -> str:
        """Return the four key words as a compact hexadecimal value."""

        return "".join(f"{int(value):08X}" for value in self.key_data)

    @property
    def is_valid(self) -> bool:
        """Return whether at least one key word is non-zero."""

        return any(int(value) != 0 for value in self.key_data)
...
class GuildPlayerStruct(Structure):
    ...
    def _read_name_pointer(self) -> str:
...
class GuildContextStruct(Structure):
    ...
    @property
    def h0020_ptrs(self) -> list[int]:
        return [int(cast(Any, value)) for value in self._values(self.h0020_array, c_uint32)]
    ...
    @property
    def announcement_str(self) -> str:
        return _format_encoded_text(self.announcement_encoded_str)
    ...
    @property
    def announcement_author_str(self) -> str:
        return _format_encoded_text(self.announcement_author_encoded_str)

    @property
    def factions_outpost_guilds(self) -> list[TownAllianceStruct]:
        return cast(
    ...
    @property
    def player_guild_history(self) -> list[GuildHistoryEventStruct]:
        return [
            value
    ...
    @property
    def guild_array(self) -> list[GuildStruct]:
        return [
            value
            for value in GWArrayView(
    ...
    @property
    def h0318_ptrs(self) -> list[int]:
        return [int(cast(Any, value)) for value in self._values(self.h0318_array, c_uint32)]

    @property
    def h032C_ptrs(self) -> list[int]:
        return [int(cast(Any, value)) for value in self._values(self.h032C_array, c_uint32)]

    @property
    def player_roster(self) -> list[GuildPlayerStruct]:
        return [
            value
            for value in GWArrayView(
...
def get() -> GuildContextStruct | None:
    ...
    return client.read_guild_context() if client is not None else None
