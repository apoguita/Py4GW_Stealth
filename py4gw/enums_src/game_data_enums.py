"""Port of Reforged's ``Py4GWCoreLib/enums_src/GameData_enums.py``.

The game's own data enumerations: ranges, dye colours, gender, professions, allegiance, ailments,
damage types, weapon types, skill types, attributes and inscriptions, plus the attribute experience
table. Every class, every member, every value and every name table is the source's, spelling
included — ``Profession._None``, ``ProfessionShort._``, ``WeaporReq`` (the source's own typo) and
``None_`` are all as written, because a ported script imports them by those names.

**Source:** ``Py4GW_Reforged/Py4GWCoreLib/enums_src/GameData_enums.py`` (485 lines). The file name
is lowercased here per ``docs/STYLE.md``; the package keeps the source's ``enums_src``. Nothing is
re-numbered, renamed, added or aliased, and the two members that carry behaviour
(``Attribute.get_profession``, ``Attribute.is_primary``) are the source's bodies.

**One member raises, and it is named:** ``DyeColor.from_dye_info`` imports the native binding type
``PyItem.DyeInfo`` to type-check its argument (``GameData_enums.py:38-48``), and this project has no
``PyItem`` binding module. It refuses through :func:`_unported` rather than accepting a look-alike
object, which would be a different member rather than this one.
"""
...
def _unported(member: str, requirement: str) -> NotImplementedError:
    ...
    The same shape ``py4gw/player.py``, ``py4gw/map.py``, ``py4gw/dialog.py`` and
    ``py4gw/agent.py`` use: name the member, name exactly what it needs, and state that the
    declaration is deliberate. The source's member works; this is this port's work item.
    """
...
class DyeColor(IntEnum):
    ...
    def from_dye_info(dye_info) -> "DyeColor":
        """``GameData_enums.py:37-48``: a dye record's first colour, or ``NoColor``.

        The source's first two lines are ``from PyItem import DyeInfo`` and an
        ``isinstance(dye_info, DyeInfo)`` test, so the member is written against a native binding
        type this project has no ported home for. Accepting anything else in its place would answer
        for a different type, so it refuses and names what it needs.
        """

        raise _unported(
            "DyeColor.from_dye_info",
            "PyItem.DyeInfo (GameData_enums.py:38-41), the native binding type the source "
            "type-checks its argument against",
        )
