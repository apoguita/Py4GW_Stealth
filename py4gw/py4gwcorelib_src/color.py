"""Port of Reforged's ``Py4GWCoreLib/py4gwcorelib_src/Color.py``.

**Source:** ``Py4GWCoreLib/py4gwcorelib_src/Color.py`` — 414 lines, two types: ``Color``
(``Color.py:3-288``) and the ``ColorPalette`` enum (``Color.py:291-414``).

Every member is present with the source's name, signature and arithmetic. The module is pure
Python — it touches no context, no client function and no binding — so nothing in it is adapted
for this project's execution model and nothing raises: the bytes, the packing orders and the
clamping are the source's, transcribed line for line.

The classes are what ``Utils`` reaches: ``Utils.NormalToColor`` returns a ``Color``
(``Utils.py:92-98``), which is why this module landed with it.
"""
...
class Color:
    """An RGBA colour with the source's two orders: ``ABGR`` and DirectX ``ARGB`` (``Color.py:3``)."""
